...
case class VacationResponseUpdateFailure(exception: Throwable) extends VacationResponseUpdateResult {
...
class VacationResponseSetMethod @Inject() (serializer: Serializer,
  ...
  private def update(mailboxSession: MailboxSession, vacationResponseSetRequest: VacationResponseSetRequest): SMono[VacationResponseUpdateResult] = {
    vacationResponseSetRequest.parsePatch()
      .flatMap(patch => patch.asVacationPatch)
      .fold(e => SMono.just(VacationResponseUpdateFailure(e)),
  ...
  private def createResponse(invocation: Invocation,
                             ...
                             updateResult: VacationResponseUpdateResult): Invocation = {
