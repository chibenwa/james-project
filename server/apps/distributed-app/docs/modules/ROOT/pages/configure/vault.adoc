...
| enabled
| Whether to enable the deleted message vault. Boolean, defaults to false.

| workQueueEnabled
| Boolean, defaults to false. Whether or not to to enable the workqueue to asynchronously process deleted items. If enabled,
processing bigger deletes in a time efficient and reliable manner becomes possible, but the use of deduplication is required.
