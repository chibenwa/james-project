...
class PR813Test {
    ...
    @Test
    void assertRemoteDeliveryDoNotAffectUTF8Subject(DockerMockSmtp mockSmtp) throws Exception {
        ...
        try {
            ...
            smtpClient.sendShortMessageData(IOUtils.toString(ClassLoader.getSystemResourceAsStream("test.eml")));
        } finally {
            smtpClient.disconnect();
        }

        calmlyAwait.atMost(TEN_SECONDS).untilAsserted(() -> assertThat(mockSmtp.getConfigurationClient().listMails())
            .extracting(Mail::getMessage)
            .singleElement()
            .satisfies(s -> assertThat(s).contains("Subject: =?UTF-8?B?Z29uZyB6aSB0aWFvYeWwgQ==?=\r\n" +
